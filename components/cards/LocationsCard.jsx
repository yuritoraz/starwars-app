...
const LocationsCard = ({ locations }) => {
  if (!location || !locations.imageUrl) {
    return null;
  }
  ...
  return (
    <div className="relative cursor-pointer transition-all duration-300 hover:scale-105">
      ...
      <div className="grid text-lg items-center absolute inset-0 bg-black bg-opacity-90 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-lg overflow-hidden">
        <div className=''>
          <h2 className="px-4 text-center text-xl font-bold text-yellow-300 mb-2">{locations.name}</h2>
          <p className='px-6 text-blue-400 text-[8px] text-justify justify-center'>{locations.description}</p>
        </div>
      </div>
    </div>
  );
