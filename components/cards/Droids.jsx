...
import DroidsCard from "./DroidsCard";
import { Skeleton } from "@heroui/react";

const Droids = ({ searchTerm = "" }) => {
  const [droids, setDroids] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchAllDroids = async () => {
      try {
        let allDroids = [];
        let page = 1;
        let hasMore = true;
        ...
        while (hasMore) {
          const response = await fetch(
            `https://starwars-databank-server.vercel.app/api/v1/droids?page=${page}&limit=50`
          );
  ...
  const filteredDroids = droids.filter((droid) =>
    droid.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
